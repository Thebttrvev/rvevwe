/* ══ BACKEND CONFIG ══ */
// เปลี่ยนเป็น URL ของ Render service หลัง deploy
...
const TEACHER_PIN = '5678'; // ← เปลี่ยนได้
...
function smartLogin() {
  ...
  // Admin login via username field
  ...
  // Teacher login (read-only admin view)
  ...
  if (!found) {
    document.getElementById('login-err').textContent = '✗ ไม่พบ Username นี้ — กรุณาสมัครก่อน';
    return;
  }
  if (found.pin !== pw) {
